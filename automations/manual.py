...
async def login(page, username: str, password: str) -> None:
    ...
    await page.goto("https://www.imarkportal.com/ecmvc/portal/auth/login?ReturnUrl=%2fecmvc")
    ...
    try:
        ...
        await page.wait_for_selector('text="Reset Filters"', state='attached', timeout=15000)  # Adjust the timeout as needed
...
async def process_item(page, upc: str, filename: str, row_index: int, total_packs, pack_size) -> None:
    ...
    try:
        ...
        btn_links = await page.locator('.general-info-content.flex-centered-vertical .btn-link').element_handles()
        for btn_link in btn_links:
            # Get the parent item card of each .btn-link
            item_card = await btn_link.query_selector('xpath=ancestor::div[contains(@class, "general-info-content")][contains(@class, "flex-centered-vertical")]')
            ...
            if not out_of_stock:
                ...
                item_name = await order_item(page, total_packs, pack_size)  # Proceed to order the item
            ...
            if not btn_links:
                ...
                update_csv_cell(filename, row_index, 'out_of_stock_reason', 'product_oss')
...
async def order_item(page, total_packs, pack_size):
    ...
    case_container = page.locator('text="Case"').locator('xpath=ancestor::div[contains(@class, "flex-centered-vertical")][contains(@class, "margin-left-8")]')
    each_container = page.locator('text="Each"').locator('xpath=ancestor::div[contains(@class, "flex-centered-vertical")][contains(@class, "margin-left-8")]')
    ...
    item_text = await page.locator(selector).first.inner_text()  # Get text from the first matching element
    print('csv pack_size:', pack_size, 'csv total_packs:', total_packs, 'web unit_size:', unit_size, 'total_units_needed:', total_units_needed, 'cases_needed:', cases_needed)
    if case_count > 0 and each_count > 0:
        ...
        print(f"Setting 'Case' orders to {cases_needed} and 'Each' orders to {each_needed}")
        # Fill in the "Each" quantity
        if each_needed:
            await each_container.locator('.dx-texteditor-input').fill(str(max(3, each_needed)))
        ...
        await page.wait_for_timeout(3000)
        overlay_yes_count = await page.locator('.dx-button.dx-button-success.dx-button-mode-contained.dx-widget.button-left-indent.dx-button-has-text.dx-dialog-button >> text="Yes"').count()
        if overlay_yes_count > 0:
            print('Trying to click on "Yes"')
            await page.locator('.dx-button.dx-button-success.dx-button-mode-contained.dx-widget.button-left-indent.dx-button-has-text.dx-dialog-button >> text="Yes"').click()
            print("Clicked on 'Yes'.")
        else:
            print("The overlay does not exist.")


        print(f"Set 'Case' orders to {cases_needed} and 'Each' orders to {each_needed} successfully.")
    else:
    ...
    try:
        ...
        modal = await page.wait_for_selector('order-type-select-item', state='visible', timeout=5000)
...
def ensure_second_column_name_ordered(filename):
    with open(filename, mode='r', newline='') as file:
        ...
        headers = [header.strip() for header in next(reader)]  # Trim whitespace from headers
        data = list(reader)
...
def remove_csv_whitespace(filename):
    ...
    with open(filename, mode='r', newline='') as file:
        reader = csv.reader(file)
        headers = [header.strip() for header in next(reader)]  # Remove whitespace from headers
...
async def main() -> None:
    async with async_playwright() as playwright:
        ...
        fileName = "./bruh/coremark1.csv"
        username = "021192617"
        password = "Dp#617"

        fileName = "./bruh/coremark2.csv"
        username = "021191569"
        password = "Dp#569"

        fileName = "./bruh/coremark3.csv"
        username = "021192625"
        password = "Dp#625"
